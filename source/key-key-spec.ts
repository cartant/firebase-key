...
describe("firebase-key", () => {
  describe("key", () => {
    ...
    it("should return the key for a Date", () => {
      expect(key(new Date(1466642894992), "min")).to.equal(
        "-KKuid1F------------"
      );
    });
    ...
    it("should support 'random'", () => {
      ...
      expect(key(1466642894992)).to.not.match(
        /^-KKuid1F(------------|zzzzzzzzzzzz)$/
      );

      expect(key(1466642894992, "random")).to.match(
        /^-KKuid1F[-_0-9A-Za-z]{12}$/
      );
      expect(key(1466642894992, "random")).to.not.match(
        /^-KKuid1F(------------|zzzzzzzzzzzz)$/
      );
    });
